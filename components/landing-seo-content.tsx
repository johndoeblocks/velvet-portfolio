    ...
    <>
      <section className="px-6 py-20 sm:py-24">
        <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-[1.05fr_0.95fr]">
          <article className="rounded-[2rem] border border-slate-900/10 bg-white p-8 shadow-[0_24px_70px_-50px_rgba(15,23,42,0.28)] sm:p-10">
          ...
          <div className="grid gap-5">
            <article className="rounded-[1.75rem] border border-slate-900/10 bg-[#fffaf3] p-6 shadow-[0_20px_60px_-45px_rgba(15,23,42,0.24)]">
          ...
          <div className="mt-12 grid gap-5 lg:grid-cols-[0.95fr_1.05fr]">
            <div className="rounded-[2rem] border border-slate-900/10 bg-white p-8 shadow-[0_24px_70px_-50px_rgba(15,23,42,0.28)] sm:p-10">
              <ul className="space-y-4">
                {service.benefits.map((benefit) => (
                  <li key={benefit} className="flex items-start gap-3 text-sm leading-relaxed text-slate-600">
                    <span className="mt-0.5 text-[#0f4c5c]">✓</span>
                    <span>{benefit}</span>
                  </li>
                ))}
              </ul>
            ...
            <div className="grid gap-5 md:grid-cols-2">
              {service.solutions.slice(0, 4).map((solution) => (
                <article
                  key={solution.title}
                  className="rounded-[1.75rem] border border-slate-900/10 bg-[#fffaf3] p-6 shadow-[0_20px_60px_-45px_rgba(15,23,42,0.24)]"
                >
                  <h3 className="text-lg font-semibold tracking-tight text-slate-950">
                    {solution.title}
                  </h3>
        ...
        <div className="mx-auto max-w-5xl">
          ...
          <div className="mt-12 space-y-4">
            {service.faqs.map((faq) => (
              <details
                key={faq.question}
                className="group overflow-hidden rounded-[1.5rem] border border-slate-900/10 bg-white shadow-[0_20px_60px_-45px_rgba(15,23,42,0.22)]"
              >
                <summary className="flex cursor-pointer list-none items-center justify-between gap-6 px-6 py-5">
                  <h3 className="text-left text-base font-semibold tracking-tight text-slate-950 sm:text-lg">
                    {faq.question}
                  </h3>
                ...
                <div className="border-t border-slate-900/8 px-6 py-5 text-sm leading-relaxed text-slate-600">
                  {faq.answer}
                </div>
              </details>
            ))}
